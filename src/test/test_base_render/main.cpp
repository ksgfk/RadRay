...
public:
    ...
    void Start() {
        GlobalInitGlfw();
        window = std::make_unique<GlfwWindow>(RADRAY_APPNAME, 1280, 720);
        ...
        cmdPool = cmdQueue->CreateCommandPool().value();
        cmdBuffer = cmdPool->CreateCommandBuffer().value();
