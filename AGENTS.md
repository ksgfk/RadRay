...
- 字符串拼接相关必须用`fmt`库, 注意类型是否具有`format_to`函数
- flag style的enum用`enum_flags.h`头文件, 已经实现了`format_to`格式化
## Test
- 各个模块的测试用例存入模块文件夹下的`tests`内，例如`core`的在`modules/core/tests/`
- 测试在`CMakeLists.txt`内用`radray_add_test`函数，需要特殊环境变量定义`RADRAY_PROJECT_DIR`、`RADRAY_TEST_ENV_DIR`等用`radray_add_radray_gtest_case`
- CTest `ctest --preset win-x64 -R BuddyAllocatorTest --output-on-failure` 注意-R后面要跟代码里写的gtest test suit名字，而不是cmake里定义的名
- 不要并发跑构建和测试
