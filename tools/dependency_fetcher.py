...
RELEASE_NOTES_MAX_CHARS = 8000
PRERELEASE_TAG_RE = re.compile(r"(?:alpha|beta|preview|pre|rc)", re.IGNORECASE)
...
def write_manifest_if_changed(path: Path, value: dict[str, Any]) -> None:
...
def github_repo_from_url(url: str) -> GitHubRepo | None:
    raw = url.strip()
...
def github_release_source_from_url_template(url_template: str) -> GitHubReleaseSource | None:
    parsed = urllib.parse.urlparse(url_template.strip())
    ...
    return GitHubReleaseSource(repo=repo, tag_template=urllib.parse.unquote(parts[4]))


def release_tag_from_version(tag_template: str, version: str) -> str:
    return tag_template.replace("{Version}", version)


def version_from_release_tag(tag_template: str, tag: str) -> str | None:
    if "{Version}" not in tag_template:
        return tag
    prefix, suffix = tag_template.split("{Version}", 1)
    if not tag.startswith(prefix) or (suffix and not tag.endswith(suffix)):
        return None
    end = len(tag) - len(suffix) if suffix else len(tag)
    return tag[len(prefix) : end]


def normalize_release_tag_request(tag_template: str, requested: str) -> tuple[str, str | None]:
    tag = requested.strip()
    if not tag:
        raise FetchError("empty release tag")
    version = version_from_release_tag(tag_template, tag)
    if version is not None:
        return tag, version
    if "{Version}" in tag_template:
        return release_tag_from_version(tag_template, tag), tag
    return tag, tag


def github_release_from_response(repo: GitHubRepo, value: dict[str, Any]) -> GitHubRelease:
    tag = str(value.get("tag_name", "")).strip()
    if not tag:
        raise FetchError(f"{repo.slug}: GitHub release response did not include tag_name")
...
def github_api_json(path: str) -> dict[str, Any]:
    ...
    if shutil.which("gh") is not None:
        try:
            ...
            if isinstance(value, dict):
                return value
            raise FetchError(f"GitHub API response is not an object: {api_path}")
        except (FetchError, json.JSONDecodeError) as exc:
            gh_error = f"\n  gh api failed first: {exc}"
    ...
    try:
        ...
        raise FetchError(f"GitHub API request failed: {url}\n  HTTP {exc.code}: {detail}{gh_error}") from exc
    except urllib.error.URLError as exc:
        raise FetchError(f"GitHub API request failed: {url}\n  {exc}{gh_error}") from exc
    except json.JSONDecodeError as exc:
        raise FetchError(f"GitHub API response was not valid JSON: {url}{gh_error}") from exc

    if not isinstance(value, dict):
        raise FetchError(f"GitHub API response is not an object: {url}{gh_error}")
    return value


def github_latest_release(repo: GitHubRepo) -> GitHubRelease:
    return github_release_from_response(repo, github_api_json(f"/repos/{repo.slug}/releases/latest"))


def github_release_by_tag(repo: GitHubRepo, tag: str) -> GitHubRelease:
    encoded_tag = urllib.parse.quote(tag, safe="")
    return github_release_from_response(repo, github_api_json(f"/repos/{repo.slug}/releases/tags/{encoded_tag}"))


def is_github_release_not_found(exc: FetchError) -> bool:
    return "api.github.com/repos/" in str(exc) and "HTTP 404" in str(exc)


def print_release_note(release: GitHubRelease) -> None:
    title = release.name if release.name != release.tag_name else release.tag_name
    print(f"  release: {title} ({release.tag_name})")
    ...
    if not body:
        print("  release note: <empty>")
        return
    if len(body) > RELEASE_NOTES_MAX_CHARS:
        body = body[:RELEASE_NOTES_MAX_CHARS].rstrip() + "\n  ... <truncated>"
    print("  release note:")
    for line in body.splitlines():
        print(f"    {line}")


def print_default_ref_note(repo_url: str, remote_commit: str) -> None:
    repo = github_repo_from_url(repo_url)
    print(f"  remote HEAD: {remote_commit}")
    if repo is not None:
        print(f"  commits: https://github.com/{repo.slug}/commits/{remote_commit}")


def print_branch_ref_note(repo_url: str, branch: str, remote_commit: str) -> None:
    parsed = urllib.parse.urlparse(repo_url)
    print(f"  remote branch: {branch}")
    print(f"  commit: {remote_commit}")
    repo = github_repo_from_url(repo_url)
    if repo is not None:
        print(f"  commits: https://github.com/{repo.slug}/commits/{branch}")
    elif parsed.hostname == "gitlab.com":
        path = parsed.path.strip("/")
        if path.lower().endswith(".git"):
            path = path[:-4]
        print(f"  commits: https://gitlab.com/{path}/-/commits/{branch}")


def print_git_tag_note(repo_url: str, tag: GitRemoteTag) -> None:
    repo = github_repo_from_url(repo_url)
    print(f"  git tag: {tag.name}")
    print(f"  commit: {tag.commit}")
    if repo is not None:
...
def hash_url(url: str) -> str:
    ...
    try:
        ...
        detail = exc.read().decode("utf-8", errors="replace").strip()
        raise FetchError(f"failed to hash remote file: {url}\n  HTTP {exc.code}: {detail}") from exc
    except urllib.error.URLError as exc:
        raise FetchError(f"failed to hash remote file: {url}\n  {exc}") from exc
    return digest.hexdigest()
...
def tag_suffix_pattern(tag: str) -> str | None:
    match = re.match(r"^v?\d+(?:[._-]\d+)*(?:[A-Za-z]\d*)?(-[A-Za-z].+)$", tag)
    if not match:
        return None
...
def sdk_artifacts(manifest: dict[str, Any], only: set[str] | None = None) -> list[dict[str, Any]]:
    ...
    for artifact in artifacts:
        ...
        if name in seen:
            raise FetchError(f"duplicate SDK artifact name: {name}")
        seen.add(name)
        if only and name not in only:
            continue
...
class GitDependencyFetcher:
    ...
    def check_release_updates_manifest(self, manifest: dict[str, Any]) -> int:
        update_count = 0
        sources = manifest_git_sources(manifest, self.only)
        if not sources:
            note("no git third-party dependencies matched")
        ...
        for source in sources:
            ...
            if kind in {"default", "branch"}:
                try:
                    remote_commit = self.resolve_source_commit(source)
                except FetchError as exc:
                    print(f"{name}: failed to query remote {ref_label(source)}: {exc}", file=sys.stderr)
                    continue
                current_commit = self.current_commit_for_update_check(source)
                if current_commit and current_commit.lower() == remote_commit.lower():
                    print(f"{name}: up to date ({ref_label(source)} at {short_commit(remote_commit)})")
                    continue
                update_count += 1
                current_label = short_commit(current_commit) if current_commit else "not installed/locked"
                print(f"{name}: update available ({ref_label(source)} {current_label} -> {short_commit(remote_commit)})")
                if kind == "branch":
                    print_branch_ref_note(str(source.get("Git", "")), value, remote_commit)
                else:
                    print_default_ref_note(str(source.get("Git", "")), remote_commit)
                continue

            if kind != "tag":
                print(f"{name}: skipped, {ref_label(source)} is not managed by GitHub release update")
                continue

            repo = github_repo_from_url(str(source.get("Git", "")))
            if repo is None:
                print(f"{name}: skipped, Git remote is not a GitHub repository")
                continue

            tag_pattern = tag_suffix_pattern(value)
            if tag_pattern:
                release = None
                try:
                    remote_tag = self.latest_remote_stable_tag(str(source.get("Git", "")), name, tag_pattern)
                except FetchError as exc:
                    print(f"{name}: failed to query latest remote tag: {exc}", file=sys.stderr)
                    continue
                latest_tag = remote_tag.name
                latest_label = f"{remote_tag.name} ({short_commit(remote_tag.commit)})"
            else:
                release = self.try_github_latest_release(repo)
                if release is not None:
                    latest_tag = release.tag_name
                    latest_label = release.tag_name
                else:
                    try:
                        remote_tag = self.latest_remote_stable_tag(str(source.get("Git", "")), name)
                    except FetchError as exc:
                        print(f"{name}: failed to query latest remote tag: {exc}", file=sys.stderr)
                        continue
                    latest_tag = remote_tag.name
            ...
            if value == latest_tag:
                print(f"{name}: up to date ({value})")
                continue

            update_count += 1
            print(f"{name}: update available ({value} -> {latest_label})")
            if release is not None:
                print_release_note(release)
            else:
                print_git_tag_note(str(source.get("Git", "")), remote_tag)
        return update_count
    ...
    def update_release_manifest(
        ...
        if not sources:
            note("no git third-party dependencies matched")
            return
        if requested_tag and len(sources) != 1:
            raise FetchError("updating to a specific tag requires exactly one dependency selected with --only")
        ...
        for source in sources:
            ...
            if kind in {"default", "branch"}:
                if requested_tag:
                    raise FetchError(f"{name}: cannot update {ref_label(source)} to a release tag")
                remote_commit = self.resolve_source_commit(source)
                current_commit = self.current_commit_for_update_check(source)
                if current_commit and current_commit.lower() == remote_commit.lower():
                    note(f"{name}: already locked/installed at {ref_label(source)} {short_commit(remote_commit)}")
                else:
                    current_label = short_commit(current_commit) if current_commit else "not installed/locked"
                    note(f"{name}: updating {ref_label(source)} {current_label} -> {short_commit(remote_commit)}")
                    if kind == "branch":
                        print_branch_ref_note(str(source.get("Git", "")), value, remote_commit)
                    else:
                        print_default_ref_note(str(source.get("Git", "")), remote_commit)
            ...
            if kind != "tag":
                if requested_tag:
                    raise FetchError(f"{name}: cannot update {ref_label(source)} to a release tag")
                warn(f"{name}: skipped, {ref_label(source)} is not managed by GitHub release update")
                continue

            repo = github_repo_from_url(str(source.get("Git", "")))
            if repo is None:
                warn(f"{name}: skipped, Git remote is not a GitHub repository")
                continue

            release: GitHubRelease | None
            remote_tag: GitRemoteTag | None = None
            tag_pattern = None if requested_tag else tag_suffix_pattern(value)
            if requested_tag:
                release = self.try_github_release_by_tag(repo, requested_tag)
                if release is None:
                    remote_tag = self.remote_tag(str(source.get("Git", "")), requested_tag, name)
                    latest_tag = remote_tag.name
                else:
                    latest_tag = release.tag_name
            elif tag_pattern:
            ...
            old_ref = ref_label(source)
            if source.get("Tag") == latest_tag and "Branch" not in source and "Commit" not in source:
                note(f"{name}: already uses {latest_tag}")
            else:
                source["Tag"] = latest_tag
                manifest_changed = True
                note(f"{name}: manifest updated {old_ref} -> tag {latest_tag}")
                if release is not None:
                    print_release_note(release)
        ...
        lock_data = self.lock_data_for_update(manifest, lock_names)
        if manifest_changed:
            write_manifest_if_changed(manifest_path, manifest)
        else:
            note("manifest already up to date")
        self.write_updated_lockfile(lock_data)
        self.reload_lockfile()
        self.install_updated_sources(manifest, install_names)

    def lock_data_for_update(self, manifest: dict[str, Any], changed_names: set[str]) -> dict[str, Any] | None:
        ...
        for source in sources:
            ...
            if name in changed_names:
                note(f"{name}: updating lockfile entry")
                entries.append(self.lock_entry_for_source(source))
            elif name in old_entries:
                entries.append(old_entries[name])
            else:
                note(f"{name}: adding missing lockfile entry")
                entries.append(self.lock_entry_for_source(source))
    ...
    def write_updated_lockfile(self, lock_data: dict[str, Any] | None) -> None:
        if lock_data is None:
            return
        write_json_if_changed(self.lockfile, lock_data)
        note(f"lockfile written: {self.lockfile}")

    def reload_lockfile(self) -> None:
        self.lock_data = read_lockfile(self.lockfile) if self.use_lock and self.lockfile.exists() else None
        self.lock_entries = lock_entries_by_name(self.lock_data)

    def install_updated_sources(self, manifest: dict[str, Any], names: set[str]) -> None:
        for source in manifest_git_sources(manifest, names):
            self.fetch_one(self.source_for_install(source))

    def current_commit_for_update_check(self, source: dict[str, Any]) -> str | None:
        name = str(source["Name"])
        if self.use_lock:
            entry = self.lock_entries.get(name)
            commit = str(entry.get("commit", "")).strip() if entry else ""
            if commit:
                return commit

        target = self.target_root / name
        if not target.exists() or not git_ok(target, "rev-parse", "--is-inside-work-tree"):
            return None
        try:
            self.validate_existing_repo(target, str(source.get("Git", "")), name)
            return self.current_head(target)
        except FetchError:
            return None
    ...
    def try_github_latest_release(self, repo: GitHubRepo) -> GitHubRelease | None:
        try:
            return github_latest_release(repo)
        except FetchError as exc:
            if not is_github_release_not_found(exc):
                warn(f"{repo.slug}: GitHub releases/latest unavailable, falling back to git tags: {exc}")
            return None

    def try_github_release_by_tag(self, repo: GitHubRepo, tag: str) -> GitHubRelease | None:
        try:
            return github_release_by_tag(repo, tag)
        except FetchError as exc:
            if not is_github_release_not_found(exc):
                warn(f"{repo.slug}: GitHub release for tag {tag} unavailable, falling back to git tag: {exc}")
            return None

    def latest_remote_stable_tag(self, repo_url: str, name: str, pattern: str = "refs/tags/v*") -> GitRemoteTag:
        output = run(["git", "ls-remote", "--tags", "--sort=-v:refname", repo_url, pattern])
        for line in output.splitlines():
            ...
            if tag.endswith("^{}"):
                tag = tag[:-3]
            if not tag or PRERELEASE_TAG_RE.search(tag):
                continue
            return GitRemoteTag(name=tag, commit=self.resolve_remote_tag(repo_url, tag, name))
        raise FetchError(f"{name}: no stable remote tags found")

    def remote_tag(self, repo_url: str, tag: str, name: str) -> GitRemoteTag:
        return GitRemoteTag(name=tag, commit=self.resolve_remote_tag(repo_url, tag, name))
...
class SdkArtifactFetcher:
    ...
    def status_manifest(self, manifest: dict[str, Any]) -> int:
        artifacts = sdk_artifacts(manifest, self.only)
        if not artifacts:
            note("no SDK artifacts matched")
            return 0
        ...
        for artifact in artifacts:
            ...
            info = self.match_artifact(artifact, current_platform, current_arch)
            if info is None:
                print(f"{name:24} skipped         no artifact for {current_platform}-{current_arch}")
                continue
            status, detail, problem = self.status_one(info)
            if problem:
                problem_count += 1
            print(f"{name:24} {status:14} {detail}")
        return problem_count
    ...
    def check_release_updates_manifest(self, manifest: dict[str, Any]) -> int:
        ...
        for artifact in artifacts:
            ...
            source = github_release_source_from_url_template(str(artifact.get("Url", "")))
            if source is None:
                print(f"{name}: skipped, URL is not a GitHub release asset template")
                continue

            current_tag = release_tag_from_version(source.tag_template, version)
            try:
                release = github_latest_release(source.repo)
            except FetchError as exc:
                print(f"{name}: failed to query latest GitHub release: {exc}", file=sys.stderr)
                continue
            latest_version = version_from_release_tag(source.tag_template, release.tag_name)
            if latest_version is None:
                print(
                    f"{name}: latest release tag {release.tag_name} does not match URL tag template "
                    f"{source.tag_template}"
                )
                continue
            ...
            update_count += 1
            print(f"{name}: update available ({version} -> {latest_version}, tag {release.tag_name})")
            print_release_note(release)
        return update_count
    ...
    def update_release_manifest(
        ...
        if not artifacts:
            note("no SDK artifacts matched")
            return
        if requested_tag and len(artifacts) != 1:
            raise FetchError("updating to a specific tag requires exactly one SDK selected with --only")

        changed = False
        install_artifacts: list[dict[str, Any]] = []
        for artifact in artifacts:
            name = str(artifact["Name"])
            source = github_release_source_from_url_template(str(artifact.get("Url", "")))
            if source is None:
                warn(f"{name}: skipped, URL is not a GitHub release asset template")
                continue

            release: GitHubRelease
            latest_version: str | None
            if requested_tag:
                release_tag, requested_version = normalize_release_tag_request(source.tag_template, requested_tag)
                release = github_release_by_tag(source.repo, release_tag)
                latest_version = requested_version or version_from_release_tag(source.tag_template, release.tag_name)
            else:
                release = github_latest_release(source.repo)
                latest_version = version_from_release_tag(source.tag_template, release.tag_name)
            ...
            if latest_version is None:
                raise FetchError(
                    f"{name}: release tag {release.tag_name} does not match URL tag template {source.tag_template}"
                )

            old_version = str(artifact.get("Version", "")).strip()
            if old_version == latest_version:
                note(f"{name}: already uses {latest_version}")
            else:
                artifact["Version"] = latest_version
                note(f"{name}: manifest updated {old_version} -> {latest_version}")
                print_release_note(release)
                self.update_artifact_hashes(artifact)
                changed = True
            install_artifacts.append(artifact)

        if not install_artifacts:
            note("nothing to update")
            return
        if changed:
            write_manifest_if_changed(manifest_path, manifest)
            ...
            note("manifest already up to date")
        self.install_updated_artifacts(install_artifacts)

    def install_updated_artifacts(self, artifacts: list[dict[str, Any]]) -> None:
        current_platform, current_arch = current_triplet()
        note(f"host triplet: {current_platform}-{current_arch}")
        for artifact in artifacts:
            info = self.match_artifact(artifact, current_platform, current_arch)
    ...
    def update_artifact_hashes(self, artifact: dict[str, Any]) -> None:
        name = str(artifact["Name"])
        triplets = artifact.get("Triplets")
        if not isinstance(triplets, list) or not triplets:
            raise FetchError(f"{name}: missing Triplets")
    ...
    def status_one(self, info: dict[str, Any]) -> tuple[str, str, bool]:
        ...
        if not stamp_file.exists() or not extract_dir.exists():
            return "missing", f"{detail} is not installed", True
        if not archive_path.exists():
            return "archive-missing", f"{detail} archive is missing: {archive_name}", True
        if expected_hash:
            actual_hash = hash_file(archive_path).lower()
            if actual_hash != expected_hash:
                return "hash-mismatch", f"{detail} hash mismatch", True
        try:
            self.validate_files(extract_dir, info)
        except FetchError as exc:
            return "invalid", str(exc), True
