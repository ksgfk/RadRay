...
const char* SHADER_CODE = R"(#define M_PI (3.14159265359)
...
ConstantBuffer<PreCameraData> _Camera : register(b2);
...
MaterialData SampleMaterial(float2 uv)
{
    MaterialData matData;
    // Pull base color and packed metallic/roughness straight from bound textures.
    float4 albedoSample = _AlbedoTex.Sample(_LinearSampler, uv);
    float4 mrSample = _MetallicRoughnessTex.Sample(_LinearSampler, uv);
    matData.albedo = albedoSample.rgb;
    matData.metallic = mrSample.r;
    matData.specularTint = float3(1.0, 1.0, 1.0);
    matData.specular = 0.5;
    matData.roughness = mrSample.g;
    matData.anisotropy = 0.0;
    matData.ior = 1.5;
    return matData;
}
...
float4 PSMain(PS_INPUT psIn) : SV_Target
{
    ...
    MaterialData matData = SampleMaterial(psIn.uv);
    DSPBRData dspbr = get_dspbr_data(matData);
...
TEST(DXC, BasicReflection) {
    ...
    auto vsRefl = dxc->GetDxilReflection(ShaderStage::Vertex, vs.Refl).value();
    auto psRefl = dxc->GetDxilReflection(ShaderStage::Pixel, ps.Refl).value();
