...
[shader("pixel")] float4 PSMain() : SV_Target0 { return RecordView.Color * MaterialValues.Color; }
    ...
    for (uint32_t index = 0; index < kRecordDraws; ++index) {
        ...
        record.Plan = static_cast<uint32_t>(fixture.Snapshot.DrawPlans.size());
        auto& plan = fixture.Snapshot.DrawPlans.emplace_back();
        plan.NormalState = plan.MirroredState = static_cast<uint32_t>(fixture.Snapshot.StatePlans.size());
        auto& state = fixture.Snapshot.StatePlans.emplace_back().State;
        plan.Program = program.Get();
        plan.Geometry = 0;
        auto& draw = fixture.Snapshot.GeometryBindingPlans[plan.Geometry];
        draw.Geometry = &fixture.Geometry;
        draw.LayoutId = layout;
        draw.IndexCount = 6;
        state.Primitive.Cull = render::CullMode::None;
        state.Primitive.FaceClockwise = (index / 256) % 2 ? render::FrontFace::CW : render::FrontFace::CCW;
