...
//   3. 内容可以比 AssetManager 活得久 —— AssetContent 自持 recycler 指针的全部理由。
//
...
protected:
    ...
    TextureContentRef MakeTexture(AssetManager& assets, std::string_view name) {
    ...
    StaticMeshContentRef MakeMesh(AssetManager& assets, uint32_t bufferCount) {
        GpuMesh mesh;
...
TEST_F(AssetContentTest, TextureUnloadKeepsContentAliveWhileHeld) {
    TextureContentRef content = MakeTexture(Assets(), "held");
    ...
    EXPECT_EQ(content->GetRefCount(), 2u) << "asset's one + this local one";
    ...
    EXPECT_EQ(content->GetRefCount(), 1u);
...
TEST_F(AssetContentTest, TextureReleasesEveryViewThroughTheRecycler) {
    TextureContentRef content = MakeTexture(Assets(), "views");
    ASSERT_TRUE(content.HasValue());
...
TEST_F(AssetContentTest, TextureContentOutlivesTheAssetManagerItself) {
    // 【为何这条必须成立】: AssetContent 自持 recycler 指针就是为它。若归零时回头向
    // AssetManager 索取 recycler, 这里会解引用一个已析构的对象。
    CountingRecycler recycler;
    TextureContentRef held;
    {
        AssetManager assets;
...
TEST_F(AssetContentTest, MeshUnloadKeepsContentAliveWhileHeld) {
    StaticMeshContentRef content = MakeMesh(Assets(), /*bufferCount*/ 2);
    ASSERT_TRUE(content.HasValue());
    ...
    EXPECT_EQ(content->GetRefCount(), 2u);
...
/// 在最后一个 shared_ptr 归零处立即析构 —— 不等 fence。归零交给基类后, 到
/// ReleaseRenderResources 必然是唯一所有者, 无需也不该再判断。
TEST_F(AssetContentTest, MeshReleasesEveryBufferThroughTheRecyclerEvenWhenShared) {
    StaticMeshContentRef content = MakeMesh(Assets(), /*bufferCount*/ 3);
    ASSERT_TRUE(content.HasValue());

    // 制造"多个持有者"这个从前会走进坏分支的局面。
    StaticMeshContentRef second = content;
    auto asset = make_unique<StaticMesh>(content);
    EXPECT_EQ(content->GetRefCount(), 3u);
