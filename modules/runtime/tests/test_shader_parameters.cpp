...
TEST(RadRayRuntimeShaderParameters, TypeTreePacksNestedArraysAndMatrices) {
...
TEST(RadRayRuntimeShaderParameters, MultipleCBuffersUseExplicitPayloadOwners) {
...
TEST(RadRayRuntimeShaderParameters, SharedPayloadOwnerKeepsQualifiedLeavesDistinct) {
    for (const shader::ShaderTarget target :
         {shader::ShaderTarget::DXIL, shader::ShaderTarget::SPIRV}) {
...
TEST(RadRayRuntimeShaderParameters, ScopedStoragePreservesQualifiedParameterIdentity) {
    for (const shader::ShaderTarget target :
         {shader::ShaderTarget::DXIL, shader::ShaderTarget::SPIRV}) {
        const auto artifact = DecodeGeneric("shared_cbuffer_type", target);
        ASSERT_TRUE(artifact.has_value());
...
TEST(RadRayRuntimeShaderParameters, ExactResourceNamePrecedesSameNamedLeafShorthand) {
    for (const shader::ShaderTarget target :
         {shader::ShaderTarget::DXIL, shader::ShaderTarget::SPIRV}) {
...
TEST(RadRayRuntimeShaderParameters, ReferencedPayloadRootCanAlsoOwnAnotherDeclaration) {
    for (const shader::ShaderTarget target :
         {shader::ShaderTarget::DXIL, shader::ShaderTarget::SPIRV}) {
...
TEST(RadRayRuntimeShaderParameters, LeafArrayIsExposedAsTypedVectorElements) {
...
TEST(RadRayRuntimeShaderParameters, InvalidParameterWritesAreTransactional) {
    const auto artifact = DecodeGeneric("nested_types");
    ASSERT_TRUE(artifact.has_value());
    const auto layout = ShaderParameterLayout::Create(artifact.value());
    ASSERT_TRUE(layout.has_value());
...
TEST(RadRayRuntimeShaderParameters, AmbiguousLeafRequiresQualifiedParameterPath) {
...
TEST(RadRayRuntimeShaderParameters, DeclarationModifierMakesOneCBufferDynamicOnBothTargets) {
...
TEST(RadRayRuntimeShaderParameters, LayoutResolveFailsClosedOnRecipeContradictions) {
...
TEST(RadRayRuntimeShaderParameters, ExplicitPolicyRefusesPlacementModifiers) {
