...
public:
    ...
    ~StaticMeshContent() noexcept override;
...
using StaticMeshContentRef = AssetContentRef<StaticMeshContent>;

/// 静态网格资产。只做标识与槽位, 数据在 StaticMeshContent。
class StaticMesh : public Asset {
public:
    explicit StaticMesh(StaticMeshContentRef content) noexcept;
    ...
    /// 存住一份 ContentRef —— 它缓存的 DrawData* 正是靠这份引用才不悬垂。
    StaticMeshContentRef AcquireContent() const noexcept { return _content; }

    /// 内容是否仍挂在本槽位上。OnUnload 之后为 false。
    bool HasContent() const noexcept { return _content.HasValue(); }

private:
    StaticMeshContentRef _content;
};
