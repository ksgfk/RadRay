...
///   ResolveVariant 与绘制绑定需要 device 支撑的变体和 binding-plan 缓存。
...
public:
    ...
    void SetShader(StreamingAssetRef<ShaderAsset> shader) noexcept;

    // ─── render queue ───
    int32_t GetRenderQueue() const noexcept { return _renderQueue; }
    void SetRenderQueue(int32_t queue) noexcept;
    void SetRenderQueue(RenderQueue queue) noexcept { SetRenderQueue(static_cast<int32_t>(queue)); }
    ...
    void SetRenderState(const MaterialRenderState& state) noexcept;
    ...
    /// 查有效属性值。优先返回 override, 否则返回 shader 默认值。
    ...
    void ClearOverride(std::string_view name) noexcept;
    ...
    /// 生成带 per-primitive 覆盖层 (MaterialPropertyBlock) 的快照 (对应 Unity 的
    /// Renderer.SetPropertyBlock)。材质层和覆盖层保持分离，由 binding resolver 按顺序应用；
...
private:
    void SetOverride(std::string_view name, MaterialPropertyValue value) noexcept;
