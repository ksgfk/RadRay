...
/// bundle.xml 清单的常驻表示。pugixml DOM 文档本身是后备存储: 读取 = 解析 + 校验 + 在 DOM
...
public:
    ...
    /// <bundle> 根元素。未加载时返回空节点。
    pugi::xml_node Root() const noexcept;

    /// 按 GUID / 存储形态的 bundle 内相对路径查条目节点。未命中返回 nullopt。
    /// path 查表按大小写不敏感口径 (Windows 上两个只差大小写的 path 指向同一文件)。
    std::optional<pugi::xml_node> FindByGuid(const AssetId& id) const noexcept;
    std::optional<pugi::xml_node> FindByPath(std::string_view relPath) const noexcept;
    ...
    /// 编程错误, abort。
    pugi::xml_node AppendEntry(std::string_view type, const AssetId& guid, std::string_view relPath);
...
private:
    ...
    unordered_map<AssetId, pugi::xml_node> _byGuid;
    /// key: 小写折叠后的存储形态 path。查找口径与写入口径一致 (大小写不敏感)。
    unordered_map<string, pugi::xml_node> _byPath;
    bool _loaded{false};
};
