...
4. **external / extract / final access / external access mode** 作为图内外边界单独建模
...
- external / attachment / copy / present / final access 仍然必须留在 graph 层
...
| Pass Semantic Boundary | graph / pass builder API | import / extract / attachment / copy / resolve / present / final access | shader 资源类别判断 |
...
建议 `ShaderPass` 类的 builder 以如下对象之一作为 schema 来源：

- `render::PipelineState*`
- `render::RootSignature*`
- 某个可以稳定返回 `BindingLayout` 的包装对象
...
```cpp
class RgShaderPassBuilderBase : public RgPassBuilderBase {
public:
...
即便公开 API 变成 `Bind(...)`，编译器内部仍然应继续走现有那种 pass access 模型：
...
只是这些调用不再由用户直接手写，而是由 shader slot schema 自动 lower。
...
| --- | --- | --- |
| `CBuffer` | `ReadBuffer` | 只读 |
| `BufferSrv` | `ReadBuffer` | 只读 |
| `TextureSrv` | `ReadTexture` | 只读 |
| `AccelerationStructure` | `ReadBuffer` 或单独 `ReadAccelerationStructure` 预留 | 初版可先按只读资源对待 |
| `BufferUav` | `ReadWriteBuffer` | 保守处理 |
| `TextureUav` | `ReadWriteTexture` | 保守处理 |
...
- imported texture
- 初始 access 由 surface / frame acquire 语义给出
...
1. external access mode 只允许 imported / extracted 资源进入
2. external access mode 只允许只读 access
3. graph-created 资源如果要在图外继续使用，必须通过 `Extract*`
4. external 资源必须在导入时给出初始 access 或明确 `Unknown`
5. extracted 资源必须收口 final access
...
3. `RgShaderPassBuilderBase` 负责把 `Bind(...)` 自动降成底层 `Read/Write/ReadWrite`
4. attachment / external / final access 语义继续直接落在 graph builder 上
...
- **render graph 决定 attachment / external / final access / copy / present**

更工程化一点说：

1. **不要** 让用户为 shader-visible resource 再手写一次 `ReadTexture/WriteBuffer`
2. **不要** 让 HLSL 反射去承载 attachment / external / final access 这类图边界语义
