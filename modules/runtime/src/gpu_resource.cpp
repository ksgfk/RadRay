...
Nullable<render::GraphicsPipelineState*> PipelineStateCache::GetOrCreateGraphics(
    ...
    // 【裸指针即可】: layout 由 key.Program 持有的那份 SharedPipelineLayout 引用保活, 而
    // Program 又由本条目的 Ref 保活 (见 GraphicsEntry::Ref)。资产为空 (调用方传了无效 ref)
    // 时 Program 的存活由调用方负责, 同理。
    SharedPipelineLayout* layoutRef = key.Program->GetSharedPipelineLayout().Get();
    render::PipelineLayout* layout = layoutRef != nullptr ? layoutRef->Get() : nullptr;
    if (layout == nullptr) {
