...
// 后端无关的 shader 变体缓存实现.
// miss 时逐 stage 编译 (Vulkan 走 SPIR-V) -> 反射 -> CreateShader -> SetGuid -> layout.
class ShaderVariantCacheImpl final : public ShaderVariantCache {
public:
    ShaderVariantCacheImpl(Device* device, Dxc* dxc, ShaderBindingLayoutCache* layoutCache) noexcept
        : _device(device), _dxc(dxc), _layoutCache(layoutCache) {}
    ~ShaderVariantCacheImpl() noexcept override { Clear(); }

    bool IsValid() const noexcept override {
        return _device != nullptr && _dxc != nullptr && _layoutCache != nullptr;
    }
    ...
    Nullable<const CompiledShaderVariant*> GetOrCreate(const ShaderVariantDescriptor& desc) noexcept override {
        ...
        for (const auto& stage : desc.Stages) {
            unique_ptr<Shader> shader = CompileStage(stage, desc, isSpirv);
            if (shader == nullptr) {
                return nullptr;
            }
...
private:
    ...
    unique_ptr<Shader> CompileStage(
...
Nullable<unique_ptr<ShaderVariantCache>> CreateShaderVariantCache(
    ...
    return unique_ptr<ShaderVariantCache>{make_unique<ShaderVariantCacheImpl>(device, dxc, layoutCache).release()};
}
