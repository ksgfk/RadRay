...
std::mutex g_vulkanLogMutex;
LogCollector* g_vulkanTestLogs = nullptr;

void _SetVulkanTestLogs(LogCollector* logs) noexcept {
    std::lock_guard<std::mutex> lock{g_vulkanLogMutex};
    g_vulkanTestLogs = logs;
}

void _VulkanTestLogCallback(LogLevel level, std::string_view message, void* userData) {
    std::lock_guard<std::mutex> lock{g_vulkanLogMutex};
    auto* fallback = static_cast<LogCollector*>(userData);
    auto* target = g_vulkanTestLogs != nullptr ? g_vulkanTestLogs : fallback;
    LogCollector::Callback(level, message, target);
}

class VulkanTestEnvironment final : public ::testing::Environment {
public:
    void SetUp() override {
        ...
        instanceDesc.LogCallback = &_VulkanTestLogCallback;
        instanceDesc.LogUserData = &_logs;
        auto instanceOpt = InstanceVulkan::InitEnv(instanceDesc);
        if (!instanceOpt.HasValue()) {
            _reason = "InstanceVulkan::InitEnv failed";
            return;
        }
        _instance = instanceOpt.Release();
    }

    void TearDown() override {
        _SetVulkanTestLogs(nullptr);
    ...
    bool IsReady() const noexcept { return _instance != nullptr; }

    string GetFailureReason() const {
        if (!_reason.empty()) {
            return _reason;
        }
        return "Vulkan test environment is not initialized";
    }

private:
    Nullable<InstanceVulkan*> _instance{};
    LogCollector _logs{};
    string _reason{};
};

VulkanTestEnvironment* _RegisterVulkanTestEnvironment() {
    auto env = make_unique<VulkanTestEnvironment>();
    auto* envPtr = env.get();
    ::testing::AddGlobalTestEnvironment(env.release());
    return envPtr;
}

[[maybe_unused]] VulkanTestEnvironment* g_vulkanTestEnvironment = _RegisterVulkanTestEnvironment();
#endif
...
bool _CreateDeviceForBackend(
    ...
    switch (backend) {
        case TestBackend::D3D12: {
            ...
            factoryDesc.IsEnableGpuBasedValid = true;
            factoryDesc.LogCallback = &LogCollector::Callback;
            factoryDesc.LogUserData = logs;
            auto factoryOpt = DXGIFactory::Create(factoryDesc);
            if (!factoryOpt.HasValue()) {
                _StoreReason(reason, "DXGIFactory::Create failed");
                return false;
            }
            auto factory = factoryOpt.Release();
            D3D12DeviceDescriptor desc{};
            desc.Factory = factory.get();
            desc.AdapterIndex = std::nullopt;
            auto deviceOpt = Device::Create(desc);
            if (!deviceOpt.HasValue()) {
                _StoreReason(reason, "Device::Create(D3D12) failed");
                return false;
            }
        ...
        case TestBackend::Vulkan: {
            ...
            if (g_vulkanTestEnvironment == nullptr || !g_vulkanTestEnvironment->IsReady()) {
                _StoreReason(reason, g_vulkanTestEnvironment != nullptr ? g_vulkanTestEnvironment->GetFailureReason() : "Vulkan test environment is not registered");
                return false;
            }
            _SetVulkanTestLogs(logs);
            ...
            auto deviceOpt = Device::Create(deviceDesc);
            if (!deviceOpt.HasValue()) {
                _StoreReason(reason, "Device::Create(Vulkan) failed");
                _SetVulkanTestLogs(nullptr);
                return false;
            }
            device = deviceOpt.Release();
...
#else
            RADRAY_UNUSED(logs);
            _StoreReason(reason, "SPIRV-Cross backend is not enabled for this build");
            return false;
#endif
#else
            RADRAY_UNUSED(logs);
