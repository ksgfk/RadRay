...
if ($Solution) {
	...
	$slnCandidates = @(Get-ChildItem -LiteralPath $BuildDir -File | Where-Object { $_.Extension -in '.sln', '.slnx' } | Sort-Object FullName)
	if ($slnCandidates.Count -eq 0) { Write-Err "在 $BuildDir 顶层未找到 .sln/.slnx"; exit 1 }
	if ($slnCandidates.Count -gt 1) {
		Write-Warn "顶层找到多个 .sln/.slnx，使用第一个: $($slnCandidates[0].FullName)"
	}
