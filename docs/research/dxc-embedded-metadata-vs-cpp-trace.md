...
1. 如果目标只是“shader ABI 元数据有一个真相源、DXIL/SPIR-V 使用同一份输出、删除人工 manifest 握手”，**small DXC frontend fork + canonical sidecar 是可行候选**。它应在 AST 仍存活的 frontend 结束边界收集 HLSL author metadata，使用现有 `DXC_OUT_EXTRA_OUTPUTS`/`IDxcExtraOutputs` 返回独立 sidecar；不新增 `DXC_OUT_KIND`，不把 sidecar 伪装成 bytecode 内嵌数据。
...
`C:\Users\ksgfk\AppData\Local\Temp\opencode\dxc-v1.9.2607`
...
调查只读了 RadRay、UE checkout 和 DXC checkout，并使用官方 DXC/GitHub 与 Khronos 一手资料；没有构建或运行实验 binary。
...
`include/dxc/Support/dxcapi.impl.h:293-372` 有内部 `DxcExtraOutputs` 实现，`DxcResult::SetOutputObject` 在 `:533-542` 可以把一个 `IDxcExtraOutputs` 对象挂到现有 output kind。但当前 `dxcompilerobj.cpp` 只设置 object、reflection、root signature、shader hash、errors/remarks 等 standard outputs；没有设置 `DXC_OUT_EXTRA_OUTPUTS` 的 custom metadata output。
...
这条路径不新增 `DXC_OUT_KIND`，也不要求 sidecar 进入最终 bytecode。`IDxcExtraOutputs` 是现有公开接口；生成 custom output 的 frontend/compiler wiring 仍属于 fork 内容。
...
sidecar 可以同时输出 `DeclaredContract` 与 `ActiveReflectionSummary`，但两者必须分字段、分来源、分校验方向。把 declaration metadata 叫成 active resource reflection 会重新制造错误的单向握手。
...
- sidecar header 包含当前 compile identity 和 schema version。
...
**Go**：目标限于 HLSL 作者声明的单一 ABI、双 backend canonical output、删除人工 manifest；愿意维护 DXC fork并分发匹配的 decoder/validator策略。
