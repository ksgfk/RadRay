...
> - 锚点: `shaderlib/core/math.hlsli`, `shaderlib/core/color.hlsli`, `shaderlib/core/frame.hlsli`, `shaderlib/core/platform.hlsli`, `shaderlib/bsdf/principled.hlsli`, `shaderlib/lighting/lights.hlsli`, `shaderlib/shadow/filtering.hlsli`, `shaderlib/passes/forward.hlsl`, `shaderlib/passes/depth.hlsl`, `shaderlib/passes/compute.hlsl`

# shaderlib

`shaderlib/` 是 HLSL 源码树，也是 compiler include 解析使用的逻辑根目录。共享数学、材质、
光照和阴影原语位于库层；根 `.hlsl` pass 位于 `passes/`，由 compiler discovery 推导 entry
topology 和 keyword domain。作者不维护 sidecar metadata，active binding 与 type tree 随
concrete target lane 的 compiler artifact 生成。
...
| `passes/` | forward、depth-only、compute 根 pass | 只通过 `core/platform.hlsli` 使用 target gate |
...
| `passes/forward.hlsl` | texture/sampler graphics pass 与颜色转换 |
...
共享结构必须保持 CPU 镜像的字段顺序、对齐和矩阵约定；对应的 CPU 断言放在 render/runtime
测试中。`shadow/filtering.hlsli` 中的序列化枚举值和 `lights.hlsli` 中的上限属于 ABI，
不能因为重命名或排版而改变。
...
验证三条产品 pass 的双 target compile、active binding 和 stage visibility。DXIL 的 `tN`、`sN`
