...
imgui          → runtime, radray_imgui         仅 RADRAY_ENABLE_IMGUI=ON；runtime 不依赖 imgui
```
...
| 可选 ImGui 模块、平台窗口、图内图片与配置 | [Runtime ImGui](runtime-imgui.md) |
...
| 帧内扩展 | `ApplicationExtension` / `Application::AddExtension` | `modules/runtime/include/radray/runtime/application_extension.h` |
| 可选 UI | `ImGuiSystem::Install` | `modules/imgui/include/radray/imgui/imgui_system.h` |
